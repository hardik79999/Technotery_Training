...
def update_routs(current_user, uuid):
    ...
    try:
        ...
        if current_user['role'] == "employee":
            ...
            # Agar task manager ko assign hua hai (By Admin)
            if task_assigned_to == current_user['id']:
                if not new_status: 
                    return jsonify({"message": "You can only update 'status' for tasks assigned to you!"}), 400
                if new_title or new_description: 
                    return jsonify({"message": "Not Allowed! You can only update 'status'."}), 403
            ...
            # Agar task manager ne kisi employee ko assign kiya hai
            elif task_assigned_by == current_user['id']:
                if new_title: 
