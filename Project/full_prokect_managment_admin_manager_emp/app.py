...
from routs.dashboard_data import get_dashboard_data_route

# ---------Migrate  AND SQLAlchemy database-------------------------
...
# 1. Database ka naya connection string (SQLAlchemy format)
app.config['SQLALCHEMY_DATABASE_URI'] = f"mysql+mysqlconnector://{os.getenv('user')}:{os.getenv('password')}@{os.getenv('host')}/{os.getenv('database')}"
